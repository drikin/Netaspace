#!/bin/bash

# Fix production deployment with complete files
SERVER_HOST="153.125.147.133"
SERVER_USER="ubuntu"
SSH_KEY="~/.ssh/id_ed25519"
APP_DIR="/home/ubuntu/backspace-fm-app"

echo "Fixing production deployment..."

# Create complete deployment package
tar czf production-fix.tar.gz \
    Dockerfile \
    package.json \
    package-lock.json \
    tsconfig.json \
    vite.config.ts \
    tailwind.config.ts \
    postcss.config.js \
    docker-compose.yml \
    client/ \
    server/ \
    shared/ \
    scripts/ \
    --exclude=node_modules \
    --exclude=.git \
    --exclude=dist

# Transfer to server
scp -i $SSH_KEY production-fix.tar.gz $SERVER_USER@$SERVER_HOST:$APP_DIR/

ssh -i $SSH_KEY $SERVER_USER@$SERVER_HOST << 'FIX_EOF'
    cd /home/ubuntu/backspace-fm-app
    
    # Stop all containers
    docker-compose down 2>/dev/null || true
    docker stop $(docker ps -aq) 2>/dev/null || true
    
    # Extract complete application
    tar xzf production-fix.tar.gz
    rm production-fix.tar.gz
    
    # Set production environment
    cat > .env << 'ENV_EOF'
NODE_ENV=production
DATABASE_URL=postgresql://neondb_owner:npg_GFeXV6cr7anp@ep-hidden-thunder-a65mlh9x.us-west-2.aws.neon.tech/neondb?sslmode=require
SESSION_SECRET=backspace_session_secret_2024
ENV_EOF
    
    # Clean Docker and rebuild
    docker system prune -f
    docker-compose build --no-cache
    docker-compose up -d
    
    # Wait for startup
    sleep 30
    
    # Check status
    docker-compose ps
    docker-compose logs backspace-fm
FIX_EOF

rm production-fix.tar.gz

# Test deployment
sleep 10
curl -s https://neta.backspace.fm/api/version

echo "Production fix completed."