...
async function fetchUrlInfo(url) {
  return new Promise((resolve) => {
    const options = {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; Backspace.fm/1.0)'
      }
    };
...
const server = createServer(async (req, res) => {
  ...
  if (pathname === '/api/topics' && req.method === 'POST') {
    ...
    if (existingTopic) {
      ...
      res.end(JSON.stringify({ error: 'Topic with this URL already exists' }));
      return;
    }
  ...
  // Topic status update (admin only)
  ...
  if (pathname.startsWith('/api/topics/') && pathname.endsWith('/star') && req.method === 'POST') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ success: true }));
    return;
  }

  if (pathname.startsWith('/api/topics/') && pathname.endsWith('/star') && req.method === 'DELETE') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ success: true }));
    return;
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Complete production server running on http://0.0.0.0:${PORT}`);
